...
trait AuthenticatesAndRegistersUsers
{
    ...
    public function postLogin(Request $request)
    {
        ...
        function diff_in_minutes_2($t1, $t2)
        {
            ...
            $total_interval = oto1_2($interval_minutes) * oto1_2($interval_hours) * oto1_2($interval_days) * oto1_2($interval_months) * oto1_2($interval_years);
        ...
        $get_username = $request->email;
        $verify_user = DB::table('users')->where('email', $get_username)->pluck('id');
        if (isset($verify_user)) {
            ...
            $last_failed_attempt_time = DB::table('users')->where('email', $get_username)->pluck('last_failed_attempt');
            $total_interval = diff_in_minutes_2($timestamp, $last_failed_attempt_time);
            if ($total_interval >= $allowed_timeout) {
                DB::table('users')->where('email', $get_username)->update(['failed_attemps' => 0]);
                DB::table('users')->where('email', $get_username)->update(['lock_user' => 'Y']);
            }
            if (DB::table('users')->where('email', $get_username)->pluck('lock_user') == 'X') {
                return redirect($this->loginPath())->withErrors(["You have been locked due to excessive wrong password attempts. Try logging after 1 hour or call VNA helpdesk at 402-444-4444"]); ;
            }
        }
    ...
    protected function getFailedLoginMessage($data)
    {
        ...
        function diff_in_minutes($t1, $t2)
        {
            $datetime1 = date_create($t1);
            $datetime2 = date_create($t2);
            $interval = date_diff($datetime1, $datetime2);
        ...
        if (!$check_id) {
            ...
            $total_interval = diff_in_minutes($timestamp, $last_failed_attempt_time);
            if ($total_interval >= $allowed_timeout) {
